...
public class Aerolinea{
    ...
    public void agregarCliente(Cliente cliente) {
    	this.clientes.put( cliente.getIdentficador( ), cliente );
    }
    ...
    public int venderTiquetes(String identificadorCliente, String fecha, String codigoRuta, int cantidad) {
        ...
        if (cliente != null && vuelo != null) {
            for (int i = 0; i < cantidad; i++) {
                Tiquete tiquete = new Tiquete(identificadorCliente, vuelo, cliente, i);
                Tiquete.add(tiquete);
            }
            return cantidad;
        }
        ...
        if (vuelo != null) {
            for (Tiquete tiquete : tiquetes) {
                if (tiquete.getVuelo().equals(vuelo)) {
                    tiquete.setUsado(true); // Suponiendo que la clase Tiquete tiene un atributo "usado"
                }
            }

            // Registrar el vuelo en un historial de vuelos realizados (si es necesario)
            // Por ejemplo:
            // historialVuelosRealizados.add(vuelo);

            System.out.println("Vuelo " + codigoRuta + " del " + fecha + " ha sido registrado como realizado.");
        } else {
            System.out.println("No se encontró el vuelo con código " + codigoRuta + " y fecha " + fecha);
        }
    }
    ...
    public String consultarSaldoPendienteCliente(String identificadorCliente) {
        ...
        if (cliente != null) {
            ...
            // Recorrer todos los tiquetes para encontrar los asociados al cliente
            for (Tiquete tiquete : tiquetes) {
                if (tiquete.getCliente().equals(cliente)) {
                    // Sumar el precio del tiquete al saldo pendiente
                    saldoPendiente += tiquete.getTarifa();
                }
            }

            // Retornar el saldo pendiente formateado como una cadena
            return "El saldo pendiente del cliente " + cliente.getIdentificador() + " es: $" + saldoPendiente;
        } else {
            // Retornar un mensaje de error si el cliente no existe
