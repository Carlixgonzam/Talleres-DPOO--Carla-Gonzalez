...
public class SandboxListas
{
    ...
    {
    	int[] arr = new int[listaEnteros.size()]; //nuevo arr con el mismo tamao de la original
        for (int i = 0; i < listaEnteros.size(); i++) { 
            arr[i] = listaEnteros.get(i); //se obtiene el elemento
        }
        return arr;
    }
    ...
    {
    	Iterator<Integer> itera = listaEnteros.iterator();
    	while (itera.hasNext()) {
    		if (itera.next() == valor) {
    			itera.remove();
    		}
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	...
    	for(int i= 0; i < valores.length; i++) {
    		listaEnteros.add((int) valores[i]); //los paso a enteros los trunco
    	}
    }
    ...
    public void reiniciarArregloCadenas( List<Object> objetos )
    {
    	...
    	for(int j = 0; j < objetos.size();j++) {
    		listaCadenas.add(objetos.get(j).toString());
    	}
    }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int i = 0; i < listaEnteros.size(); i++) {
            ...
            if (listaEnteros.indexOf(num) != listaEnteros.lastIndexOf(num)) { //se repite
                conta++;
            }
        }
